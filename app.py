...
def load_data():
    ...
    # 1. Load Events (Local Files Only)
    # User specified Superliga file: Superliga_GW17_2025 (Assuming .csv)
    # Maintaining Liga II previous name or generic fallback

    local_files_map = {
        'Liga II': 'LigaII_2025_2026_events.csv',
        'Superliga': 'Superliga_GW17_2025.csv'
    }
    ...
    for comp, filename in local_files_map.items():
        if os.path.exists(filename):
            df = load_and_process_wyscout(filename)
            if not df.empty:
                df['Competition'] = comp
                data_store['events'][comp] = df
        ...
        if selected_z_family in role_definitions:
            ...
            if not df_filtered.empty:
                ...
                if valid_calc:
                    ...
                    # Only add metrics that exist in the dataframe to prevent KeyError
                    valid_top_metrics = [m for m in top_metrics if m in df_filtered.columns]
                    cols_show.extend(valid_top_metrics)
