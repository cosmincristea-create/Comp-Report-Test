...
def normalize_event_columns(df):
    ...
    col_map = {
        ...
        'eventName': ['event_type', 'event_name', 'event'],
    ...
    with col_filters:
        ...
        if selected_comp in data['events']:
             ...
             if not perf_events.empty:
                 ...
                 valid_moves = perf_events[perf_events['eventName'].isin(['Pass', 'Others on the ball', 'Free Kick'])]
                 # This is heuristic. Wyscout is complex. 'Pass' is main one.
                 ...
                 # Simplified: Count events named 'Duel', 'Interception'
                 def_mask = perf_events['eventName'].isin(['Duel', 'Interception', 'Save attempt', 'Goalkeeper leaving line'])
                 # Recovery often 'Others on the ball' with specific tag, hard to parse quickly.
                 # We will count Duels + Interceptions.
